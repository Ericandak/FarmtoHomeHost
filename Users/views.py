...
def home(request):
    ...
    # Get statistics for the facts section
    active_sellers_count = User.objects.filter(is_active=True, groups__name='Seller').count()
...
def get_chatbot_response(message):
    ...
    if "track" in message and "order" in message:
        return ("To track your order:\n"
                "1. Go to 'My Orders' in your account\n"
                "2. Find your order number\n"
                "3. Click on 'Track Order' to see real-time status")
    
    elif "product information" in message:
        return ("Our products include:\n"
                "• Fresh vegetables and fruits\n"
                "• Organic produce\n"
                "• Local farm products\n"
                "Each product includes details about:\n"
                "- Price per unit\n"
                "- Farmer information\n"
                "- Product description")
    
    elif "contact seller" in message:
        return ("You can contact sellers by:\n"
                "1. Going to the product page\n"
                "2. Clicking on 'Contact Seller'\n"
                "3. Sending them a message through our secure system")
...
def test_chatbot(request):
    try:
        chatbot = ChatbotSingleton.get_instance()
        response = chatbot.get_response("Hello, can you help me with product information?")
...
def test_chatbot_connection(request):
    try:
        chatbot = ChatbotSingleton.get_instance()
        connection_success = chatbot.test_connection()
        ...
        if connection_success:
            ...
            logger.error("Chatbot connection test failed")
            return JsonResponse({
                'status': 'error',
                'message': 'Chatbot connection test failed'
            }, status=500)
    except Exception as e:
        logger.error(f"Error testing chatbot connection: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': f'Error testing chatbot connection: {str(e)}'
        }, status=500)
