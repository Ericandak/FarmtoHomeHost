...
from orders.models import UserMilestone, Order, OrderItem
from django.views.decorators.http import require_POST
from django.core.exceptions import ValidationError
import json
from .translation_service import TranslationService
from fuzzywuzzy import fuzz
from django.db.models import Q, Case, When, Value, IntegerField, Avg, Count, Exists, OuterRef
...
def home(request):
    ...
    total_orders_count = Order.objects.filter(delivery_status='delivered').count()
    average_rating = Review.objects.aggregate(Avg('rating'))['rating__avg'] or 4.5
    ...
    ).filter(Q(avg_rating__gte=4) | Q(avg_rating__isnull=True)).order_by('-created_at')[:3]
    ...
    context = {
        ...
        'has_seller_role': user.groups.filter(name='Seller').exists(),
    }
